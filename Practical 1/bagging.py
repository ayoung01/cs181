...
r = len(PARAM)
SCORE = np.zeros((r, 1)) #valid


def calc_bagged_rss(param_idx, y_hat_bagged, valid):
    rss = 0.0
    for i, entry in enumerate(valid):
        r = entry['rating']
        rss += (r - y_hat_bagged[i])**2
    SCORE[param_idx] += rss
...
for k in range(num_folds):
    ...
    for i, param in enumerate(PARAM):
        Y_HAT = []; y_hat_bagged = 0
        for bag in range(num_bags):
            # model_result 
            y_hat, train_rss = run_model(train, valid, 'prediction', param)
            Y_HAT.append(y_hat)
        for y in Y_HAT:
            y_hat_bagged += y
        y_hat_bagged = y_hat_bagged / num_bags
        calc_bagged_rss(i, y_hat_bagged, valid)
