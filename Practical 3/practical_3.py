...
f = open("sorted_common_calls", 'r')
loc = pickle.load(f) # list of all common function calls sorted in alphebetical order
f.close()
...
for datafile in file_names:
    ...
    for el in tree.iter():
        ...
        if el.tag == "all_section" and not in_all_section:
            ...
            if loc.count(el.tag) == 1:
                m[n][loc.index(el.tag)] = m[n][loc.index(el.tag)] + 1
    print n
    n = n+1
