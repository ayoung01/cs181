...
class ExampleTeamAgent(BaseStudentAgent):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ALPHA_POW = 0
        self.GAMMA = 0.1
        self.Q = np.zeros((7,4,7,4,7,4,2,3))
        # self.Q = pickle.load(open("Q","rb"))
        self.k = np.zeros((7,4,7,4,7,4,2,3)) # num times action a has been taken from state s
    ...
    def chooseAction(self, observedState):
        ...
        if self.last_action==None:
            try:# we identify the bad ghost by its first feature
                self.bad_ghost = ghost_states[ghost_quadrants.index(4)].getFeatures()[0]
            except:
                print 'there is no bad ghost??'
        ...
        # returns a default legal action
        # policy: 0 = eat good ghost, 1 = eat power capsule, 2 = eat bad ghost
        def policy_to_action(pol):
            if pol == 0:
                return good_dir
            if pol == 1:
                return cap_dir
        ...
        def default_action():
            ...
            return sanity_check(action), policy

        last_reward = curr_score - self.last_score
        
        new_action, policy = default_action()
        if not self.last_action == None: # if we're not at the very beginning of the step
            ...
            if not sum(self.Q[curr_state])==0:
                ...
                new_action = policy_to_action(np.argmax([Q_N,Q_E,Q_S]))

            self.k[curr_state][policy] += 1
            ALPHA = 1/pow(self.k[curr_state][policy], self.ALPHA_POW)
            self.Q[self.last_state][self.last_policy] += ALPHA*(last_reward+self.GAMMA*max_Q-self.Q[self.last_state][self.last_policy])
