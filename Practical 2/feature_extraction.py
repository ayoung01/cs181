...
# genre 
X_genres = np.load(open('genres.npy', 'rb'))

# company 
X_companies = np.load(open('companies.npy', 'rb'))

# rating 
X_ratings = np.load(open('ratings.npy', 'rb'))
...
X_train = np.concatenate((X_quant, X_wc, X_genres, X_companies, X_ratings, X_sentiment), axis=1)
pickle.dump((X_train, global_feat_dict, y_train, train_ids),
            open('features.p', 'wb'))
...
X_train[:, global_feat_dict['number_of_screens']] = \
        X_train[:, global_feat_dict['number_of_screens']] ** 2
X_train[:, global_feat_dict['production_budget']] = \
        X_train[:, global_feat_dict['production_budget']] ** 0.3



quant_set = [global_feat_dict['running_time'], global_feat_dict['number_of_screens'], \
                global_feat_dict['production_budget']]
df = DataFrame(np.concatenate((y_train[:, np.newaxis], X_train[mask,:][:,quant_set]), axis=1))
